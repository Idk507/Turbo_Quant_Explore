...
$$
\tilde{\mathbf{x}} = \tilde{\mathbf{x}}_{\mathrm{mse}} + \tilde{\mathbf{x}}_{\mathrm{qjl}}.
$$
...
$$
\mathbb{E}[\langle \mathbf{y}, \tilde{\mathbf{x}}_{\mathrm{qjl}} \rangle] = \langle \mathbf{y}, \mathbf{r} \rangle.
$$
...
= \operatorname{Var}\left( \langle \mathbf{y}, \tilde{\mathbf{x}}_{\mathrm{qjl}} \rangle \mid \tilde{\mathbf{x}}_{\mathrm{mse}} \right)
...
- Reduce to the uniform distribution over the sphere $S^{d-1}$.
- Any $b d$-bit quantizer partitions the sphere into at most $2^{bd}$ Voronoi cells.
- By Shannon's rate-distortion theorem for the uniform spherical source, or by a direct entropy argument, the minimal average distortion per coordinate is at least $1/4^b$, the high-rate limit for MSE on a normalized uniform source on $[-1,1]$.
- The $d$-fold product structure and orthogonality give the aggregate $1/4^b$ lower bound for MSE.
- For inner product distortion, project onto a random direction and use the same per-coordinate lower bound, yielding the extra $1/d$ factor from the variance of inner products on the sphere.
